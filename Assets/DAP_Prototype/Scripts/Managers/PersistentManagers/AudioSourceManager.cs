    ...
    public class AudioSourceManager : PersistentManager<AudioSourceManager, IAudioSourceManager>, IAudioSourceManager
    {
        private class AudioSourceFade
        {
            ...
            public bool ChangeTarget(UnityEngine.AudioSource target)
            {
                ...
                this.target = target;
                // @neuro: probably ok with PlayScheduled not respecting time already elapsed?
        ...
        )
        {
            ...
            result.name = template.name + " (managed copy)";
        ...
        )
        {
            if (audioSource != null)
            {
                ...
                if (NonDiegeticBackgroundIsPlaying() && nonDiegeticBackground.clip == audioSource.clip)
                {
                    ...
                    return;
                    // @neuro: might make sense to add an option to crossfade with
            ...
            {
                // @neuro: if already transitioning, notify and switch the transition
